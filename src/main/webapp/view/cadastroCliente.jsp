        ...
        <div class="area-fixa">
            ...
            <section id="secao-cadastro">
                <form action="controle" method="POST">
                    ...
                    <input type="hidden" name="id" value="<%if (op.equals(listarPorIdStr) && lcli != null && !lcli.isEmpty()) {
                            out.print(lcli.getFirst().getId());
                        }%>">
                    <%}%>
                    ...
                    <input type="text" name="nome" placeholder="Seu nome:" required <%if (op.equals(listarPorIdStr) && lcli != null && !lcli.isEmpty()) {
                                out.print(String.format("value='%s'", lcli.getFirst().getNome()));
                            }%>>
                    <input type="email" name="email" placeholder="Seu e-mail:" required <%if (op.equals(listarPorIdStr) && lcli != null && !lcli.isEmpty()) {
                    ...
                    <input type="tel" name="telefone" required pattern="[0-9]{2} [0-9]{5}-[0-9]{4}" maxlength=13 placeholder="Seu telefone:"
                           ...
                           oninput="this.setCustomValidity('')" <%if (op.equals(listarPorIdStr) && lcli != null && !lcli.isEmpty()) {
                    ...
                    <button type="submit"><%if (op.equals(listarPorIdStr) && !lcli.isEmpty()) {
                        ...
                        out.print(" " + target.toUpperCase());%></button>
                </form>
            </section>

            <nav id="menu-principal">
                <div class="menu-titulo">MENU</div>
                <a href="controle?model=<%out.print(targetSessao);%>&op=<%out.print(listarTodosStr);%>" class="btn-link">EM CARTAZ</a>
                <a href="controle?model=<%out.print(targetFilme);%>&op=<%out.print(listarTodosStr);%>" class="btn-link">FILMES</a>
                <a href="controle?model=<%out.print(targetSala);%>&op=<%out.print(listarTodosStr);%>" class="btn-link">SALAS</a>
                <a href="controle?model=<%out.print(target);%>&op=<%out.print(listarTodosStr);%>" class="btn-link">CLIENTES</a>
            </nav>
        </div>
    </body>
